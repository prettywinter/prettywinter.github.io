...
{% note color:green 举例： trade:order:1 %}

2. 简洁性：保证语义的前提下，控制key的长度，当key较多时，内存占用也不容忽视。
{% note color:green 举例： user:{uid}:friends:messages:{mid}简化为u:{uid}:fr:m:{mid} %}
    
3. 不要包含特殊字符。
{% note color:red 反例： 包含空格、换行、单双引号以及其他转义字符 %}
...
{% codeblock 正例 lang:bash %}
hmset user:1 name tom age 19 favor football
{% endcodeblock %}
{% endgrid %}

{% grid child:codeblock color:red %}
{% codeblock 反例 lang:bash %}
set user:1:age 19 3 
